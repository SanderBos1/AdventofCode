...
def queue_logic(data, visited, distance, q):
    both = q.popleft()
    current = both[0]
    ...
    return data, visited, distance, q
...
def Main():
    ...
    q = deque()
    q.append([s_position, find_value_S(s_position, data)])
    while(len(q) != 0):
        data, visited, distance, q = queue_logic(data, visited, distance, q)
    return distance
